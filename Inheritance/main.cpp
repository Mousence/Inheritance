	...
	if (fin.is_open()) {
		...
		while (std::getline(fin, line)) {
		   ...
		   if (Type == "Student") {
			...
			double rating;
			double attendance;
			iss >> Class >> Type >> HUMAN_GET_PARAMETERS >> STUDENT_GET_PARAMETERS;
			C_group[i] = new Student(HUMAN_GIVE_PARAMETERS, STUDENT_GIVE_PARAMETERS);
			}
		   else if (Type == "Teacher") {
			std::string speciality;
			int experience;
			iss >> Class >> Type >> HUMAN_GET_PARAMETERS >> TEACHER_GET_PARAMETERS;
			C_group[i] = new Teacher(HUMAN_GIVE_PARAMETERS, TEACHER_GIVE_PARAMETERS);
			   ...
			   double attendance;
			   std::string subject;
			   iss >> Class >> Type >> HUMAN_GET_PARAMETERS >> STUDENT_GET_PARAMETERS >> subject;
